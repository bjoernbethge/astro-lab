...
from astro_lab.config import get_config, get_survey_config, get_combined_config, get_task_config
...
def test_config_loading():
    ...
    try:
        ...
        if "recommended_model" in gaia_config:
            print(f"  ✓ Recommended model: {gaia_config['recommended_model']['conv_type']}")
    ...
    for survey in surveys:
        try:
            ...
            if "recommended_model" in config:
                ...
                print(f"✓ {survey}: {rec['conv_type']} with {rec.get('num_layers', '?')} layers")
            else:
