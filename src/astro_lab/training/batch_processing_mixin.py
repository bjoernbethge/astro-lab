...
class BatchProcessingMixin:
    ...
    def _process_chunk(self, chunk: Union[torch.Tensor, TensorDict]) -> Union[torch.Tensor, TensorDict]:
        """
    ...
    def lazy_stack_predictions(
        ...
        if isinstance(data_list[0], TensorDict):
            ...
            raise TypeError(
                f"Unsupported data type in data_list: {type(data_list[0])}"
            )

    def _stack_tensordicts(
        self, 
        tensordict_list: List[TensorDict], 
        device: Optional[torch.device] = None
    ) -> TensorDict:
