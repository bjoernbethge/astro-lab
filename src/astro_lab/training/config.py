...
class SchedulerConfig(BaseModel):
    ...
    scheduler_type: Literal["cosine", "onecycle", "plateau", "step", "exponential"] = "cosine"
...
class TrainingConfig(BaseModel):
    ...
    def __init__(self, **data):
        ...
        if self.training is not None:
            self.scheduler.learning_rate = self.training.learning_rate
            self.scheduler.weight_decay = self.training.weight_decay
            self.scheduler.max_epochs = self.training.num_epochs
            self.data.batch_size = self.training.batch_size
            self.hardware.gradient_clip_val = self.training.gradient_clip_val
    ...
    def validate_hardware_compatibility(self) -> bool:
        ...
        if self.hardware.precision in ["16", "16-mixed"] and not torch.cuda.is_available():
            return False
        ...
        return self.model.validate_device_compatibility(device)
