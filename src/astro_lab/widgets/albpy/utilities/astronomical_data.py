...
def create_sample_stellar_data(n_stars: int = 1000, spectral_classes: Optional[List[str]] = None) -> Dict[str, torch.Tensor]:
    """
...
def create_sample_galaxy_data(n_galaxies: int = 500, galaxy_types: Optional[List[str]] = None) -> Dict[str, torch.Tensor]:
    """
    ...
    type_weights = {
        "E0": 0.05, "E3": 0.08, "E7": 0.07,
        "S0": 0.15,
        "Sa": 0.10, "Sb": 0.15, "Sc": 0.20,
        "SBa": 0.05, "SBb": 0.08, "SBc": 0.12,
        "Irr": 0.15
    }
    ...
    galaxy_classes = np.random.choice(galaxy_types, size=n_galaxies, p=available_weights)
...
def create_sample_nebula_data(n_nebulae: int = 200, nebula_types: Optional[List[str]] = None) -> Dict[str, torch.Tensor]:
    """
