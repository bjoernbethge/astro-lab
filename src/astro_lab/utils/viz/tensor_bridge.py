...
try:
    ...
    # Monkey patch PyVista PolyData to prevent __del__ errors
    try:
...
class PyVistaZeroCopyBridge(ZeroCopyBridge):
    ...
    @staticmethod
    def to_pyvista(
        tensor: torch.Tensor, scalars: Optional[torch.Tensor] = None, **kwargs
    ) -> "pv.PolyData":
        ...
        with zero_copy_context("PyVista conversion"):
            ...
            points_optimized = PyVistaZeroCopyBridge.ensure_cpu_contiguous(
                points_tensor
            )
            ...
            try:
                ...
                if scalars is not None:
                    scalars_optimized = PyVistaZeroCopyBridge.ensure_cpu_contiguous(
                        scalars
                    )
    ...
    def to_pyvista_safe(
        ...
        with pyvista_mesh_context():
            return PyVistaZeroCopyBridge.to_pyvista(tensor, scalars, **kwargs)

    @staticmethod
    def cleanup_pyvista_mesh(mesh: "pv.PolyData"):
...
class BlenderZeroCopyBridge(ZeroCopyBridge):
    ...
    def to_blender(
        ...
        # Based on: https://salaivv.com/2023/07/07/context-mangers-blender.html
        ...
        with blender_memory_context():
            with zero_copy_context("Blender conversion"):
                ...
                data_optimized = BlenderZeroCopyBridge.ensure_cpu_contiguous(
                    points_tensor
                )
