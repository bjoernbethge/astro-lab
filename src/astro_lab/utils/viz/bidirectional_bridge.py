...
class BidirectionalPyVistaBlenderBridge:
    ...
    def pyvista_to_blender(
        ...
        try:
                ...
                for key, data in mesh.point_data.items():
                    if len(data) == len(vertices):
                        ...
                        colors = np.column_stack(
                            [data_normalized] * 3 + [np.ones_like(data_normalized)]
                        )
            ...
            logger.info(
                f"✅ Converted PyVista mesh to Blender: {name} ({len(vertices)} vertices)"
            )
            return blender_obj
    ...
    def blender_to_pyvista(
        self, obj: Any, include_materials: bool = True
        ...
        try:
            ...
            if include_materials and mesh_data.materials:
                ...
                if material and material.use_nodes:
                    ...
                    if principled:
                        ...
                        mesh.point_data["material_color"] = np.tile(
                            base_color[:3], (len(vertices), 1)
                        )
            ...
            logger.info(
                f"✅ Converted Blender object to PyVista: {obj.name} ({len(vertices)} vertices)"
            )
            return mesh
    ...
    def sync_mesh(
        ...
        try:
            ...
            if is_pyvista_source and not is_pyvista_target:
                # PyVista → Blender
                self._sync_pyvista_to_blender(
                    source, target, sync_vertices, sync_faces, sync_materials
                )
            elif not is_pyvista_source and is_pyvista_target:
                # Blender → PyVista
                self._sync_blender_to_pyvista(
                    source, target, sync_vertices, sync_faces, sync_materials
                )
            else:
...
class MaterialBridge:
    ...
    def pyvista_to_blender_material(
        pyvista_mesh: "pv.PolyData", material_name: str = "PyVistaMaterial"
    ) -> Optional[Any]:
    ...
    def blender_to_pyvista_material(
        blender_material: Any, num_vertices: int
    ) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            if blender_material and blender_material.use_nodes:
                ...
                if principled:
                    base_color = principled.inputs["Base Color"].default_value
                    material_data["material_color"] = np.tile(
                        base_color[:3], (num_vertices, 1)
                    )
...
# High-level convenience functions with proper memory management
...
def quick_convert_pyvista_to_blender_safe(
    mesh: "pv.PolyData", name: str = "converted_mesh"
) -> Optional[Any]:
    """
    Quick conversion from PyVista to Blender with proper memory management.

    Uses contextlib-based memory management as recommended by:
    https://salaivv.com/2023/07/07/context-mangers-blender.html
    """
    if bpy is None:
        raise ImportError("Blender not available")
...
__all__ = [
    ...
    "quick_convert_pyvista_to_blender_safe",
