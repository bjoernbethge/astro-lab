...
def create_data_page():
    ...
    return mo.vstack([
        ...
        mo.md("Load and process real astronomical survey data using AstroLab's data pipeline."),
        ...
        viz_component if viz_component else mo.md("⏳ **Load data first to enable visualization**")
    ])
...
def create_data_visualization_section(loaded_data):
    ...
    try:
        ...
        if not has_coordinates:
            return mo.md("⚠️ **Loaded data has no spatial coordinates for visualization**")
...
def check_for_coordinates(data):
    ...
    try:
        ...
        coord_cols = ['ra', 'dec', 'x', 'y', 'z', 'coordinates', 'galactic_l', 'galactic_b']
