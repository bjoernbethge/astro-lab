...
def create_cosmic_web_page():
    ...
    if analyzer_config and analyzer_config.get("configured"):
        try:
            analysis_status = mo.md("🔄 **Running cosmic web analysis...** Please wait.")
            ...
            if analysis_results.get("error"):
                analysis_status = mo.md("❌ **Analysis failed:** " + analysis_results['error'])
            elif analysis_results.get("analysis_complete"):
                analysis_status = mo.md("✅ **Cosmic web analysis completed successfully!**")
            else:
    ...
    return mo.vstack([
        mo.md("# 🌌 Cosmic Web Analysis"),
        mo.md("Analyze cosmic structures using AstroLab's cosmic web detection algorithms."),
...
def create_analysis_data_status(loaded_data):
    ...
    try:
        ...
        if not has_spatial:
            return mo.md("⚠️ **Loaded data lacks spatial coordinates needed for cosmic web analysis**")
