...
class ModelsModule:
    ...
    def create_model_lab(self) -> mo.Html:
        ...
        # Training controls
        controls = self._create_training_controls(model_type.value, model_selector.value)
    ...
    def _create_training_status(self) -> mo.Html:
        ...
        # Progress bar
        progress_bar = mo.Html(f"""
            <div class="progress-container">
                <label>Training Progress</label>
                <progress value="{progress}" max="100"></progress>
                <span>{progress}%</span>
            </div>
        """) if status == "training" else mo.Html("")
        
        # Training logs
        logs = state.get("training_logs", [])
        logs_display = mo.Html(f"""
            <div class="training-logs">
                <h4>Training Logs</h4>
                <pre>{'\\n'.join(logs[-10:])}</pre>
            </div>
        """) if logs else mo.Html("")
    ...
    def _create_model_info(self) -> mo.Html:
        ...
        return mo.vstack([
            ...
            mo.md(f"**Input Features:** {getattr(model, 'in_features', 'Unknown')}"),
            mo.md(f"**Output Classes:** {getattr(model, 'num_classes', 'Unknown')}"),
        ])
    ...
    def _get_registered_models(self) -> List[Dict]:
        ...
        try:
            ...
            for model in client.list_registered_models():
                latest_version = max(model.latest_versions, 
