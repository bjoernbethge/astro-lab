...
def model_selector() -> mo.Html:
    ...
    def show_model_info(model_type: str, model_name: str):
        ...
        if model_type == "preset":
            info = MODEL_PRESETS.get(model_name, {})
            set_state(lambda s: {
                **s,
    ...
    if runs:
        ...
        for run in runs:
            table_data.append({
                ...
                "val_loss": f"{run.get('metrics.val_loss', 0):.4f}" if run.get('metrics.val_loss') else "N/A",
                "val_acc": f"{run.get('metrics.val_acc', 0):.3f}" if run.get('metrics.val_acc') else "N/A",
            })
