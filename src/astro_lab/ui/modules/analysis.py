...
def analysis_panel() -> mo.Html:
    ...
    run_btn = mo.ui.button(
        label="🔬 Run Analysis", on_click=run_analysis, kind="success"
    )
...
def clustering_tool() -> mo.Html:
    ...
    def perform_clustering_analysis():
        ...
        try:
            ...
            if algorithm.value in ["dbscan", "kmeans"]:
                ...
                if algorithm.value == "spectral":
                    clusterer = SpectralClustering(
                        n_clusters=n_clusters.value, affinity="nearest_neighbors"
                    )
            ...
            if "silhouette_score" in results:
                mo.output.append(
                    mo.md(f"- **Silhouette Score:** {results['silhouette_score']:.4f}")
                )
    ...
    # Parameter display (statisch - kein direkter .value Zugriff)
    params = mo.vstack(
        [
            mo.md("**Algorithm Parameters:**"),
            n_clusters,
            mo.hstack([eps, min_samples]),
            bandwidth,
            min_cluster_size,
            mo.md("*Configure parameters based on your selected algorithm*"),
        ]
    )
...
def statistics_viewer() -> mo.Html:
    ...
    compute_btn = mo.ui.button(
        label="📊 Compute Statistics", on_click=compute_stats, kind="success"
    )
...
def subgraph_sampler() -> mo.Html:
    ...
    sample_btn = mo.ui.button(
        label="🎲 Sample Subgraph", on_click=sample_subgraph, kind="success"
    )
...
def cosmic_web_analysis_interface() -> mo.Html:
    ...
    scales_input = mo.ui.text(
        value=", ".join(
            map(str, default_scales.get(survey_select.value, [10.0, 50.0]))
        ),
        label="Clustering Scales (pc)",
    )
    ...
    def visualize_results():
        ...
        try:
            ...
            first_scale = list(
                state()["analysis_results"]["clustering_results"].keys()
            )[0]
            cluster_labels = state()["analysis_results"]["clustering_results"][
                first_scale
            ]["labels"]
            ...
            if state()["analysis_results"].get("density_analysis"):
                density_counts = state()["analysis_results"]["density_analysis"][
                    "counts"
                ]
    ...
    # Action buttons
    load_btn = mo.ui.button(label="📥 Load Data", on_click=load_data, kind="success")
    analyze_btn = mo.ui.button(
        label="🔬 Analyze Structure", on_click=analyze_structure, kind="success"
    )
    visualize_btn = mo.ui.button(
        label="🎨 Visualize Results", on_click=visualize_results, kind="neutral"
    )
    filament_btn = mo.ui.button(
        label="🌊 Analyze Filaments", on_click=analyze_filaments, kind="neutral"
    )
